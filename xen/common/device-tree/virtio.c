...
int __init dom_construct_virtio(struct kernel_info *ki)
{
    struct virtio_msg_bus_xen *bus;
    struct domain *d = ki->d;
    struct vmp *vmp;
    unsigned int i;
    int rc;

    BUILD_BUG_ON(ARRAY_SIZE(ki->virtio_mmio) != ARRAY_SIZE(d->virtio_msg_bus));

    for ( i = 0; i < ARRAY_SIZE(ki->virtio_mmio); i++ )
    {
        uint64_t addr = ki->virtio_mmio[i].addr;
        uint32_t size = ki->virtio_mmio[i].size;
        uint32_t irq = ki->virtio_mmio[i].irq;
        domid_t domid = ki->virtio_mmio[i].msg_bus.xen.device_domid;
...
fail:
    ...
    /* This destroys and free's up all registered busses and vmp clients.  */
    virtio_msg_bus_domain_destroy(d);
    return rc;
}

static int __init parse_vmp(struct dt_device_node *np, struct kernel_info *ki)
{
    ...
    dt_for_each_child_node(np, bus_np)
    {
        ...
        if ( bus_id >= ARRAY_SIZE(ki->virtio_mmio) )
        {
            printk(XENLOG_ERR "%s: bus-id %d out of bounds (max %ld)\n",
                   dt_node_full_name(bus_np), bus_id,
                   ARRAY_SIZE(ki->virtio_mmio) - 1);
            return -EINVAL;
        }

        if ( ki->virtio_mmio[bus_id].size )
        {
    ...
    /* Success.  */
    ki->virtio_mmio[bus_id].addr = addr;
    ki->virtio_mmio[bus_id].size = size;
    ki->virtio_mmio[bus_id].irq = irq;
    ki->virtio_mmio[bus_id].msg_bus.xen.device_domid = device_domid;
